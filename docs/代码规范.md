...
本文档定义了微信福帮手项目的代码规范，基于若依框架的最佳实践。
...
if (errorMsg != null && errorMsg.contains("504 Gateway Time-out")) {
    log.warn("收到504 Gateway Timeout，任务可能已提交成功");
    // 返回成功状态，让系统等待回调
...
**重要规范**：
- ✅ 业务逻辑代码默认放在 `WxFbsir-business` 模块下
- ✅ 前端业务页面默认放在 `WxFbsir-ui/src/views/business` 目录下
- ⚠️ 如需在其他位置存放业务代码，必须先与管理员探讨确认后才可以
...
```java
// ✅ 正确：使用占位符，避免字符串拼接
log.info("创建文章成功 - articleId: {}, userId: {}", articleId, userId);
log.error("调用API失败 - url: {}, error: {}", apiUrl, e.getMessage(), e);
...
详细规范请查看 [代码合并PR规范](./代码合并PR规范.md#数据库变更规范)
