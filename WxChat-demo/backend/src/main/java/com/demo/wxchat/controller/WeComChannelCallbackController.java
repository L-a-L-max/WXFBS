 ...
 */
@RestController
@RequestMapping("/v2/channel/callback/wecom/message")
public class WeComChannelCallbackController {

    private static final Logger log = LoggerFactory.getLogger(WeComChannelCallbackController.class);
    ...
    public WeComChannelCallbackController() {
    ...
    public String handleMessage(
        ...
        try {
            ...
            if (isToRealUser && content != null && !content.isEmpty() && (msgType == null || "text".equalsIgnoreCase(msgType))) {
                ...
                // 保存AI回复到数据库
                aiChatMessageService.saveAssistantMessage(toUser, content, null);
                
                // 同时存储带 web_ 前缀的版本（兼容前端不同的sessionId格式）
                if (!toUser.startsWith("web_")) {
                    aiChatMessageService.saveAssistantMessage("web_" + toUser, content, null);
                }
