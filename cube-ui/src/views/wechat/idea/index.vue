...
import { getIdeaPromptList,getIdeaPrompt,deleteIdeaPrompt,saveIdeaPrompt,updateIdeaPrompt,setIdeaPromptCommon } from "@/api/wechat/aigc";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getIdeaPromptList(this.queryParams).then(response => {
    ...
    handleUpdate(row) {
      ...
      getIdeaPrompt(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateIdeaPrompt(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            saveIdeaPrompt(this.form).then(response => {
    ...
    handleSetCommon(row, isCommon) {
      ...
      this.$modal.confirm(`是否确认${text}该思路模板？`).then(() => {
        return setIdeaPromptCommon(row.id, isCommon);
      }).then(() => {
        this.getList();
